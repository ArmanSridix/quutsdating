...
class account extends db_connect
{
    ...
    public function signup($username, $fullname, $password, $email, $sex, $year, $month, $day, $u_age, $u_sex_orientation, $language = '')
    {
        ...
        $stmt = $this->db->prepare("INSERT INTO users (free_messages_count, language, state, login, fullname, passw, email, salt, balance, bYear, bMonth, bDay, dob,bio,job,company,college,location sex, u_age, sex_orientation, regtime, accountModerateAt, accountPostModerateAt, allowMessages, admob, admob_create_at, ip_addr) value (:free_messages_count, :language, :state, :username, :fullname, :password, :email, :salt, :balance, :bYear, :bMonth, :bDay, :sex, :age, :sex_orientation, :createAt, :accountModerateAt, :accountPostModerateAt, :allowMessages, :admob, :admob_create_at, :ip_addr)");
        ...
        $stmt->bindParam(":bio", $bio, PDO::PARAM_INT);
        $stmt->bindParam(":job", $job, PDO::PARAM_INT);
    ...
    public function setdob($dob)
    {
        ...
        $stmt = $this->db->prepare("UPDATE users SET dob = (:dob) WHERE id = (:accountId)");
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
    ...
    public function setjob($job)
    {
        ...
        $stmt = $this->db->prepare("UPDATE users SET job = (:job) WHERE id = (:accountId)");
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
    ...
    public function setcompany($company)
    {
        ...
        $stmt = $this->db->prepare("UPDATE users SET company = (:company) WHERE id = (:accountId)");
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
    ...
    public function setcollege($college)
    {
        ...
        $stmt = $this->db->prepare("UPDATE users SET college = (:college) WHERE id = (:accountId)");
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
    ...
    public function get()
    {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $result = array("error" => false,
                                "error_code" => "Profile Updated Successfully",
                                "id" => $row['id'],
                                "admob" => $admob_feature,
    ...
    public function setprofilePhoto($uploadedFile)
{
    $uploadDirectory = PHOTO_PATH; // Specify your upload folder
    $uploadedFileName = $uploadedFile['name'];
    $uploadedFilePath = $uploadDirectory . $uploadedFileName;

    $url = APP_URL . '/photo/' . $uploadedFileName; // Adjust the URL construction
    ...
    if (move_uploaded_file($uploadedFile['tmp_name'], $uploadedFilePath)) {
        ...
        $stmt->bindParam(":account_id", $this->id, PDO::PARAM_INT);
        $stmt->bindParam(":originPhotoUrl", $url, PDO::PARAM_STR);
        $stmt->bindParam(":normalPhotoUrl", $url, PDO::PARAM_STR);
        $stmt->bindParam(":bigPhotoUrl", $url, PDO::PARAM_STR);
        $stmt->bindParam(":lowPhotoUrl", $url, PDO::PARAM_STR); 
