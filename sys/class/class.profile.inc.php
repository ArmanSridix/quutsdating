...
class profile extends db_connect
{
    ...
    SELECT 
        users.*,
        COALESCE(COUNT(CASE WHEN photos.itemType = 0 AND photos.fromUserId = users.id THEN 1 END), 0) AS total_photos,
        COALESCE(COUNT(CASE WHEN photos.itemType = 1 AND photos.fromUserId = users.id THEN 1 END), 0) AS total_videos
    FROM 
        users
    LEFT JOIN 
        photos ON users.id = photos.fromUserId
    WHERE 
        users.id = :id 
    LIMIT 1
    ");
        ...
        //     SELECT 
        //         users.*,
        //         COUNT(CASE WHEN photos.itemType = 0 AND photos.fromUserId = users.id THEN 1 END) AS total_photos,
        //         COUNT(CASE WHEN photos.itemType = 1 AND photos.fromUserId = users.id THEN 1 END) AS total_videos
        //     FROM 
        //         users
        //     LEFT JOIN 
        //         photos ON users.id = photos.fromUserId
        //     WHERE 
        //         users.id = :id And removeAt=0
        //     LIMIT 1
        // ");
        $stmt->bindParam(":id", $this->id, PDO::PARAM_INT);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $online = false;
                $myLike = false;
                // print_r($this->getRequestFrom());die;
                if ($this->getRequestFrom() != 0 && $this->getRequestFrom() != $this->getId()) {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                while ($row = $stmt->fetch()) {
                    ...
                    $profile->setRequestFrom($row['toUserId']);
                    // print_r($row['fromUserId']);die;
    ...
    // My New Function for like and request in one

    public function getrequestandlike($itemId = 0)
    {
        if ($itemId == 0) {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                while ($row = $stmt->fetch()) {
                    ...
                    $profile = new profile($this->db, $row[
                        
                        ($row['requestType']!='profile_i_liked'?'fromUserId':'toUserId') ]);

                    $profile->setRequestFrom($this->requestFrom);
                    $profileInfo = $profile->getVeryShort();

                    $profileInfo['requestType'] = $row['requestType'];
